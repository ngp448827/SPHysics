...
%gencase% %name% %dirout%/%name% -save:all 
...
%partvtk% -dirdata %diroutdata% -savevtk %dirout2%/PartFlap -onlytype:-all,+moving
if not "%ERRORLEVEL%" == "0" goto fail
...
%tracerparts% -dirdata %diroutdata% -savevtk %dirout2%/ColParts -onlytype:-all,+fluid -onlypos:2:-1:0.05:7:1:1 -nearpartsdist:0.04 -nearpartsdef:ptls[x=3:1:4,z=0.1:0.1:11] -tailsize:40
if not "%ERRORLEVEL%" == "0" goto fail

set dirout2=%dirout%\measuretool
%measuretool% -dirdata %diroutdata% -pointsdef:ptls[x=5:1:3,y=0:0:1,z=0.2:0.001:1000] -onlytype:-all,+fluid -elevation:0.4 -savecsv %dirout2%/_elevation -savevtk %dirout2%/elevation 
if not "%ERRORLEVEL%" == "0" goto fail

%measuretool% -dirdata %diroutdata% -pointsdef:pt=6:0:0.4 -onlytype:-all,+fluid -vars:-all,+vel -savecsv %dirout2%/_velocity -savevtk %dirout2%/velocity
if not "%ERRORLEVEL%" == "0" goto fail
